...
exports.updateBr_powdt = async (req, res) => {
  ...
  try {
    console.log("Updating Br_powdt with data:", req.body);
    ...
    const currentRecord = await Br_powdtModel.findOne({
      where: {
        refno: req.body.refno,
        product_code: req.body.product_code
      },
    ...
    // Update the record
    const result = await Br_powdtModel.update(
      updateFields,
      {
        where: {
          refno: req.body.refno,
    ...
    if (newQtySend >= originalQty) {
      console.log(`Product ${req.body.product_code} has been fully dispatched (${newQtySend}/${originalQty}), removing from br_powdt`);
      ...
      const remainingProducts = await Br_powdtModel.count({
    ...
    await t.rollback();
    console.error("Error updating br_powdt:", error);
    res.status(500).send({
      result: false,
